...
A kinematic tree is a hierarchical structure in which bodies are connected by joints. The top-level body is worldbody. 

## Body

A body is a container that defines a rigid body in MuJoCo. A body has two coordinate frames. The first is the body (reference) frame, in which the geom, site, joint, and child body are defined. The second is the inertial frame, whose origin is located at the body’s center of mass and whose axes are aligned with the principal axes of inertia

Inside a <body> tag, the following elements can be defined.
- joint
- geom
- site
- camera
...
Geometry is used to specify appearance and collision geometry. Each geom belongs to a body and is rigidly attached to that body. Multiple geoms can be attached to the same body. a geom can also have densiy or mass values specified in the XML.
